...
const Header = () => {
  ...
  const { onCopy: onCopyPrivateKey } = useClipboard(
    ...
    <Container maxW="1440" py="10">
      <Flex justifyContent="space-between" alignItems="flex-start">
        <HStack fontSize="32px">
          <Text color="second" fontSize="5xl" fontFamily="Himagsikan">
            ITOKA
          </Text>
          <Text color="white">IC-XRP</Text>
        </HStack>

        {isConnect && connectData && (
            ...
            <VStack color="gray.300" align="stretch">
              <Text title={connectData.principal}>
                ...
                <Tooltip label="copy to clip board">
                  <Link onClick={onCopy}>
                    {ellipsisShort(connectData.principal)}
                  </Link>
                </Tooltip>
              </Text>
              {connectData.xrp && (
                <Text>
                  XRP Public Key:&nbsp;
                  <Tooltip label="copy to clip board">
                    <Link onClick={onCopyPublicKey}>
                      {ellipsisKey(connectData.xrp.publicKey)}
                    </Link>
                  </Tooltip>
                </Text>
              )}
              {connectData.xrp && (
                <Text>
                  XRP Private Key:&nbsp;
                  <Tooltip label="copy to clip board">
                    <Link onClick={onCopyPrivateKey}>
                      {ellipsisKey(connectData.xrp.privateKey)}
                    </Link>
                  </Tooltip>
                </Text>
              )}
